...
echo "🎨 Enhancing active tab visibility with accent colors..."
echo "======================================================"
...
for theme_file in "$THEMES_DIR"/*.json; do
    if [ -f "$theme_file" ] && [[ ! "$(basename "$theme_file")" =~ Icons ]]; then
        ...
        # Update active tab foreground to use accent color for better visibility
        # For compressed JSON (single line files like Vira-Theme-*)
        if grep -q '"colors":{' "$theme_file"; then
            sed -i "s/\"tab\.activeForeground\":\"#FFFFFF\"/\"tab.activeForeground\":\"$accent_color\"/g" "$theme_file"
        else
...
echo "   Active tabs now use accent colors for better visibility."
echo ""
